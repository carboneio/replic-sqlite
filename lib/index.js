...
const SQLiteOnSteroid = (db, myPeerId = null, options) => {
  ...
  function closeRemotePeer (remotePeerId) {
    if (peerSockets[remotePeerId]) {
      ...
      peerStats[remotePeerId]._debounceCleanup = setTimeout(cleanDeadPeer, MAX_PEER_DISCONNECTION_TOLERANCE_MS, remotePeerId);
    }
  }
  ...
  function amITheLeader () {
    // Only perform backups if we're the master (smallest peerId among connected peers)
    // If no peers are connected, we assume we're the master
    if (nbConnectedPeers === 0) {
      return true;
    }
    for (const _peerIdStr in peerStats) {
      const _peerId = parseInt(_peerIdStr, 10);
      if (_peerId < myPeerId) {
        return false; // Another peer has a smaller ID
      }
    }
    return true; // We have the smallest ID
  }
  ...
  function onSynced (peerId) {
    ...
    if (peerStartedNotSynced.has(peerId) && _peerStat?.[LAST_MESSAGE_TIMESTAMP] > 0) {
