...
// Remote peer indexes pointer in the peerStats array for fast access (avoiding map lookup of objects, ...)
...
// other peers send ping message regularly with their last contiguous sequenceId known from me.
...
const SQLiteOnSteroid = (db, myPeerId = null, options) => {
  ...
  const MAX_PEER_DISCONNECTION_TOLERANCE_MS = options?.maxPeerDisconnectionToleranceMs ?? 1000 * 60 * 5; // 5 minutes (prevent leader flicker detection)
  ...
  // A debouncer system keep the last known state of the peer before deleting it
  const peerStats = {};
   ...
   * Backup the databas
   *
   * Trigger can be 'scheduled', 'shutdown' (nam)
  ...
  function backupDatabase (trigger = 'scheduled', callback) {
    DATABASE_BACKUP_ABSOLUTE_PATH_FN(trigger, (_backupFileName) => {
      if (!_backupFileName) {
        ...
        callback?.(new Error(`No backup file name for ${trigger} trigger`), trigger, null);
        return;
      }
  ...
  function _prepareAllStatements () {
    ...
    globalStatements.listMissingSequenceIds = (() => {
      ...
      // TODO, for now when a new peer is connected, it sends a persistent patch to fix the detection of missing patches
      // Otherwise, the other solution would be to insert fake patch (PING) to fill the missing patches (1 missing patch per peer) and make the "lead" window work.
  ...
  /**
   * Test if the database is correctly initialized and if the rowPatch is correctly stored/retrieved in/from the database.
   *
   ...
   * But it can be used (table by table) in the tests to verify that everything is working as expected (BigInt are converted to strings, blob ...)
   *
   * Same usage as `upsert`
   *
   * @param {*} tableName
   * @param {*} rowPatch
   * @param {*} callback(err) returns an error if the Replic-sqlite is not correctly initialized
   */
  ...
  /**
   * Add a remote peer to the replication system.
   *
   * The socket is expected to have a `send` method and a `on` method that accepts a `message` event listener.
   *
   * @param {number} remotePeerId
   * @param {Object} socket
   * @param {Function} socket.send (message) => void
   * @param {Function} socket.on (message : any) => void
   * @param {Function} socket.off (message : any) => void
   * @returns {void}
   */
  ...
  /**
   * Clean the stats of a remote peer. It is called after a timeout of MAX_PEER_DISCONNECTION_TOLERANCE_MS.
   *
   * @param {number} remotePeerId
   * @returns {void}
   */
  ...
  /**
   * Compute who is the leader among the connected peers.
   *
   * @returns {void}
   * @private
   */
  ...
  /**
   * Check if this peer is the leader (smallest peerId among connected peers)
   *
  ...
  function onSynced (peerId) {
    ...
    const _peerId = parseInt(peerId, 10); // peerId can be a string when coming from "for in loop"
   ...
   * @param {number} maxTime absolute maximum time to wait for the function to return true in ms
   * @param {number} delay initial delay between calls (exponential backoff) in ms
  ...
  /**
   * Check if all peers have received all patches produced by this peer
   * If a peer is not connected, it is considered as having received all patches
   * If a peer has received a patch with a sequence id greater than the last sequence id of this peer, it is considered as having received all patches
   *
   * @returns {boolean} True if all peers have received all patches produced by this peer
   */
  ...
  function exit (callback) {
    ...
    if (backupTask) {
      backupTask.stop();
      // backupTask.destroy(); // TODO test error on exit
    }
    if (!callback) {
